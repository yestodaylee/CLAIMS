...
//	  return false;
	...
	ColumnType* tmp = new ColumnType(t_u_long);
...
///*for testing end*/
	...
	for (int i = 0; i < table_descriptor_->getNumberOfProjection(); i++)
	{
		...
		part=projection_schema[i]->getcolumn(partition_key_local_index).operate->GetPartitionValue(partition_key_addr,partition_functin_list_[i]->getNumberOfPartitions(),nullptr);
